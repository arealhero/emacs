...
(defun vlad/cxx-switch-between-header-and-source-files ()
  ...
  (let* ((current-file (buffer-file-name))
         ...
         (filename (file-name-sans-extension current-file))
         (other-file (if (string-suffix-p "_ut" filename)
                         (vlad/cxx--find-corresponding-source-file (substring filename 0 (- (length filename) 3)))
                       (cond ((string-equal file-extension "c") (file-name-with-extension current-file "h"))
